...
sub makelabel
{
  ...
  if (exists $params{"LABEL"}) {
    ...
    $format = $Foswiki::cfg{Plugins}{FootNotePlugin}{LabelFormat} unless defined $format;

    # Make a new label formatter for this format.
    if (!exists $labelformats{$format}) {
      $labelformats{$format} = Foswiki::Plugins::FootNotePlugin::LabelFormat->new($format,%labelformats);
    }
